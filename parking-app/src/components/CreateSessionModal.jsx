...
export default function CreateSessionModal({ isOpen, onRequestClose, onCreate }) {
  ...
  return (
    <Modal
      ...
      className="mx-auto w-full max-w-lg rounded-lg bg-white p-6 shadow-xl outline-none"
      overlayClassName="fixed inset-0 bg-black/40 flex items-center justify-center px-4"
      contentLabel="Create Parking Session"
    >
      <h2 className="text-xl font-semibold text-gray-900">Create Parking Session</h2>
      ...
      <form className="mt-4 space-y-4" onSubmit={handleSubmit}>
        ...
        <div className="pt-2">
          <button
            ...
            className="inline-flex items-center rounded-md bg-indigo-600 px-4 py-2 text-white shadow-sm transition hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:cursor-not-allowed disabled:opacity-60"
          >
          ...
          <button
            ...
            className="ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-gray-700 shadow-sm transition hover:bg-gray-50"
          >
            Cancel
          </button>
        </div>
      ...
      <div className="mt-6">
        ...
        {loadingFavs ? (
          ...
          <ul className="mt-2 max-h-40 space-y-2 overflow-y-auto">
            {favorites.map((f) => (
              <li key={f.id} className="flex items-center justify-between rounded-md border border-gray-200 p-2">
                <div>
                  <div className="text-sm font-medium text-gray-900">{f.nickname}</div>
                  <div className="text-xs text-gray-600">
                    {f.plate} · {f.state} · {f.defaultLocation}
                  </div>
                </div>
                <button
                  className="rounded-md border border-gray-300 bg-white px-3 py-1 text-sm text-gray-700 shadow-sm hover:bg-gray-50"
